...
class TestRUCValidation(TestL10nPyBase):
    ...
    def test_ruc_check_digit_calculation(self):
        ...
        for ruc_number, expected_dv in test_cases:
            with self.subTest(ruc=ruc_number):
                from odoo.addons.l10n_py_core.validators.ruc_validator import RUCValidator
                calculated_dv = RUCValidator._calculate_check_digit(ruc_number)
                self.assertEqual(str(calculated_dv), expected_dv,
    ...
    def test_ruc_formatting(self):
        ...
        self.assertIn('-', partner.l10n_py_ruc, "RUC deve ser formatado automaticamente")
    ...
    def test_ruc_search(self):
        ...
        self.assertIn(partner, found, "Busca por RUC completo deve encontrar o parceiro")
        
        # Buscar por RUC sem formatação
        found = self.env['res.partner'].search([('l10n_py_ruc', 'ilike', '123456789')])
        self.assertIn(partner, found, "Busca por RUC sem hífen deve encontrar o parceiro")
...
class TestInvoicing(TestL10nPyBase):
    ...
    def test_invoice_number_format(self):
        ...
        self.assertTrue(re.match(number_pattern, invoice.full_invoice_number),
...
def suite():
    ...
    # Testes unitários
    test_suite.addTest(unittest.makeSuite(TestRUCValidation))
    test_suite.addTest(unittest.makeSuite(TestCDCGeneration))
    
    # Testes de integração
    test_suite.addTest(unittest.makeSuite(TestInvoicing))
    test_suite.addTest(unittest.makeSuite(TestEDIIntegration))
    test_suite.addTest(unittest.makeSuite(TestEDILogging))
