...
class TestCDCGeneration(common.TransactionCase):
    ...
    def test_cdc_structure(self):
        ...
        self.assertEqual(
            components["ruc"], self.company_ruc.zfill(8), "RUC incorreto no CDC"
        )
        self.assertEqual(
            components["doc_type"],
            f"{self.doc_type:02d}",
        ...
        self.assertEqual(
            components["expedition_point"],
            self.expedition_point,
    ...
    def test_cdc_validation_invalid_check_digit(self):
        ...
        invalid_cdc = (
            valid_cdc[:-1] + "0" if valid_cdc[-1] != "0" else valid_cdc[:-1] + "1"
        )
        ...
        self.assertIn(
            "verificador inválido", error, "Mensagem deve indicar DV inválido"
        )
    ...
    def test_cdc_validation_non_numeric(self):
        ...
        self.assertIn(
            "números", error, "Mensagem deve mencionar que CDC deve conter números"
        )
    ...
    def test_cdc_different_doc_types(self):
        ...
        unique_cdcs = set(cdcs)
        self.assertEqual(
            len(cdcs), len(unique_cdcs), "CDCs de diferentes tipos devem ser únicos"
        )
