        ...
        for journal in self:
            if journal.l10n_py_point:
                ...
                if len(journal.l10n_py_point) != 3:
                    raise ValidationError(_('El punto de expedición debe tener 3 dígitos'))
    ...
    def _check_timbrado_consistency(self):
        ...
        for journal in self:
            if journal.l10n_py_timbrado:
                if (journal.l10n_py_timbrado.establishment != journal.l10n_py_establishment or
                    journal.l10n_py_timbrado.expedition_point != journal.l10n_py_point):
                    raise ValidationError(
                        _('El timbrado seleccionado no corresponde al establecimiento y punto de expedición configurados en el diario.')
                    )
