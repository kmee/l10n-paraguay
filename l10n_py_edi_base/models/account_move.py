...
class AccountMove(models.Model):
    ...
    l10n_py_emission_type = fields.Selection([
        ('1', 'Normal'),
        ('2', 'Contingencia')
    ], string='Tipo de Emisión', default='1', required=True)
    ...
    ], string='Tipo de Transacción', required=True, default='1')
    ...
    ], string='Tipo de Presencia', default='1')
    
    # Campos de respuesta EDI
    l10n_py_cdc = fields.Char('CDC', readonly=True, copy=False,
    ...
    l10n_py_edi_status = fields.Selection([
        ...
        ('cancelled', 'Cancelado'),
        ('error', 'Error')
    ], string='Estado EDI', default='draft', readonly=True, copy=False)
    ...
    l10n_py_security_code = fields.Char('Código de Seguridad', size=9, readonly=True, copy=False)
    ...
    def _compute_edi_document_type(self):
        for move in self:
            ...
            elif move.move_type == 'in_invoice' and move.partner_id.id == move.company_id.partner_id.id:
    ...
    def _check_security_code(self):
        for record in self:
            if record.l10n_py_security_code and len(record.l10n_py_security_code) != 9:
                raise ValidationError(_('El código de seguridad debe tener exactamente 9 caracteres'))
    ...
    def _prepare_edi_document_data(self):
        ...
        document_data = {
            ...
            'fecha': self.invoice_date.strftime('%Y-%m-%dT%H:%M:%S') if self.invoice_date else fields.Datetime.now().strftime('%Y-%m-%dT%H:%M:%S'),
            ...
            'factura': {
                'presencia': int(self.l10n_py_presence_type)
            },
    ...
    def _prepare_customer_data(self):
        ...
        customer_data = {
            ...
            'nombreFantasia': getattr(partner, 'l10n_py_trade_name', False) or partner.name,
            'tipoOperacion': 1,  # B2B
        ...
        if getattr(partner, 'l10n_py_department_code', False):
            customer_data.update({
                ...
                'departamentoDescripcion': getattr(partner, 'l10n_py_department_name', False) or '',
                'distrito': getattr(partner, 'l10n_py_district_code', False) or 0,
                'distritoDescripcion': getattr(partner, 'l10n_py_district_name', False) or '',
        ...
        if getattr(partner, 'l10n_py_document_type', False):
            ...
            customer_data['documentoNumero'] = partner.vat or getattr(partner, 'l10n_py_document_number', False) or ''
    ...
    def _prepare_payment_condition(self):
        ...
        if self.invoice_payment_term_id:
            ...
            payment_condition['credito'] = {
                ...
                'plazo': f'{self.invoice_payment_term_id.line_ids[0].days} días' if self.invoice_payment_term_id.line_ids else '0 días',
            ...
            if self.invoice_date and self.invoice_payment_term_id.line_ids:
                for i, line in enumerate(self.invoice_payment_term_id.line_ids):
                    ...
                    cuotas.append({
                        'moneda': self.currency_id.name,
                        'monto': self.amount_total / len(self.invoice_payment_term_id.line_ids),
    ...
    def _prepare_invoice_lines(self):
        ...
        for line in self.invoice_line_ids.filtered(lambda l: not l.display_type):
            ...
            if iva_type == 1:  # Gravado
                ...
                iva_amount = line.price_subtotal - base_grav
            else:
                base_grav = 0
                iva_amount = 0
    ...
    def _validate_edi_data(self):
        ...
        partner = self.partner_id
        if getattr(partner, 'l10n_py_taxpayer_type', False) == '1' and not getattr(partner, 'l10n_py_ruc', False):
        ...
        if hasattr(journal, 'l10n_py_timbrado_validity') and journal.l10n_py_timbrado_validity and journal.l10n_py_timbrado_validity < fields.Date.today():
            errors.append(_('El timbrado está vencido'))
        
        # Validar productos
        for line in self.invoice_line_ids.filtered(lambda l: not l.display_type):
            if not getattr(line.product_id, 'l10n_py_ncm_code', False):
                errors.append(_('El producto %s no tiene código NCM') % line.product_id.name)
    ...
    def action_send_edi(self):
        ...
        provider = self.env['ir.config_parameter'].sudo().get_param('l10n_py.edi_provider')
        
        if provider == 'factpy':
            connector = self.env['l10n_py.edi.connector.factpy'].sudo().search([], limit=1)
        elif provider == 'facturasend':
            connector = self.env['l10n_py.edi.connector.facturasend'].sudo().search([], limit=1)
        else:
    ...
    def action_retry_edi(self):
        ...
        if self.l10n_py_edi_status not in ['error', 'rejected']:
            raise UserError(_('Solo se pueden reintentar documentos con error o rechazados'))
