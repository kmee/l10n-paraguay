...
class TestDemoData(TransactionCase):
    ...
    def test_demo_partners_exist(self):
        ...
        partner = self.env.ref("l10n_py_account.partner_contribuyente_general")
        self.assertTrue(partner)
        self.assertEqual(partner.l10n_py_ruc, "80012345")
        self.assertEqual(partner.l10n_py_taxpayer_type, "1")

        partner_svc = self.env.ref("l10n_py_account.partner_contribuyente_servicios")
        self.assertTrue(partner_svc)
        self.assertEqual(partner_svc.l10n_py_ruc, "80067890")

        partner_nc = self.env.ref("l10n_py_account.partner_no_contribuyente_ci")
    ...
    def test_demo_products_exist(self):
        ...
        product_10 = self.env.ref("l10n_py_account.product_iva_10_electronica")
        self.assertTrue(product_10)
        self.assertTrue(product_10.taxes_id)

        product_5 = self.env.ref("l10n_py_account.product_iva_5_alimento")
        self.assertTrue(product_5)

        product_exempt = self.env.ref("l10n_py_account.product_exento_libro")
        self.assertTrue(product_exempt)

    def test_demo_authorizations_exist(self):
        """Timbrados demo foram criados com ranges corretos"""
        auth_001 = self.env.ref("l10n_py_account.demo_authorization_001")
        ...
        auth_nc = self.env.ref("l10n_py_account.demo_authorization_credit_note")
        self.assertTrue(auth_nc)
        self.assertEqual(auth_nc.name, "11223344")

        auth_nd = self.env.ref("l10n_py_account.demo_authorization_debit_note")
    ...
    def test_demo_invoice_fe_iva10_amounts(self):
        ...
        invoice = self.env.ref("l10n_py_account.demo_invoice_fe_iva10")
    ...
    def test_authorization_validity_dates(self):
        ...
        auth = self.env.ref("l10n_py_account.demo_authorization_001")
        self.assertTrue(auth.date_from)
        self.assertTrue(auth.date_to)
        self.assertLess(auth.date_from, auth.date_to)

        auth_expired = self.env.ref("l10n_py_account.demo_authorization_expired")
